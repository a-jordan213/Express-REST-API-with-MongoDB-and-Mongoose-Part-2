...
.post((req, res, next) => { //add a new comment,
    Campsite.findById(req.params.campsiteId)
    .then(campsite => {
...
.put((req, res) => { //mostly stay as is, use backticks
    res.statusCode = 403;
...
.put((req, res, next) => { //update exisiting data/comment, not the author/time/ only comment text and rating
    Campsite.findById(req.params.campsiteId)
    .then(campsite => {
...
.delete((req, res, next) => { //now tryign to remove comments
    Campsite.findById(req.params.campsiteId)
    .then(campsite => {
        if (campsite && campsite.comments.id(req.params.commentId)) {
